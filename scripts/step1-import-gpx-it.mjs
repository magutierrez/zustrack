#!/usr/bin/env node
/**
 * Step 1 — Import GPX files into Supabase (Italy)
 *
 * Parses GPX files from a local directory and upserts them into the `trails`
 * table. Adapts the regex logic to Italian trail categories (GR, PR, CAI).
 *
 * Usage:
 *   node scripts/step1-import-gpx-it.mjs --dir ./gpx_trails/it --country it [--dry-run] [--limit N] [--skip-existing]
 *
 * Flags:
 *   --dir             Directory with .gpx files (default: ./gpx_trails/it)
 *   --country         Country code to store (default: it)
...
function extractTrailCode(name) {
  // Adapted for Italy: GR, PR, CAI
  const match = name.match(/\b(GR|PR|CAI)-((?:[A-Z]{1,3}-)?(?:\d+(?:\.\d+)*))/i);
