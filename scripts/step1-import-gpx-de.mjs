#!/usr/bin/env node
/**
 * Step 1 — Import GPX files into Supabase (Germany)
 *
 * Parses GPX files from a local directory and upserts them into the `trails`
 * table. Adapts the regex logic to German trail categories (GR, PR).
 *
 * Usage:
 *   node scripts/step1-import-gpx-de.mjs --dir ./gpx_trails/de --country de [--dry-run] [--limit N] [--skip-existing]
 *
 * Flags:
 *   --dir             Directory with .gpx files (default: ./gpx_trails/de)
 *   --country         Country code to store (default: de)
...
function extractTrailCode(filename, name) {
  // German filename pattern: "{CODE}. {Description}_{id}.gpx"
  // Everything before the first ". " is the trail code.
  // Handles both dash-separated (GR-3, GR-AE, GR-H1) and space-separated (GR 128, GR 56 L3).
  const prefixMatch = filename.match(/^((GR|PR)[-\s].+?)\s*\.\s/i);
  if (prefixMatch) {
  ...
  // Fallback: search inside GPX <name> field
  const nameMatch = name.match(/\b(GR|PR)[-\s]([A-Z0-9][A-Z0-9\s\-]*?)(?=\s*[-–\.]\s|\s{2,}|$)/i);
  if (nameMatch) {
    return `${nameMatch[1].toUpperCase()}-${nameMatch[2].trim().replace(/\s+/g, '-').toUpperCase()}`;
  }
  return null;
}
...
function analyzeGPX(gpxContent, filename) {
  ...
  const trailCode = extractTrailCode(filename, name);
